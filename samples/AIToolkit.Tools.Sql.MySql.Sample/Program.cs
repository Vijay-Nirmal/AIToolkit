using AIToolkit.Tools.Sql.MySql;
...
if (string.IsNullOrWhiteSpace(apiKey))
{
    ...
    Console.Error.WriteLine("dotnet user-secrets set \"OpenAI:ApiKey\" \"<your-api-key>\" --project samples/AIToolkit.Tools.Sql.MySql.Sample");
    return;
}

await EnsureSampleDatabaseAsync(adminConnectionString, databaseName);

var connectionString = BuildConnectionString(adminConnectionString, databaseName, "AIToolkit.Tools.Sql.MySql.Sample");
...
{
    var setupConnectionString = BuildConnectionString(adminConnectionString, "mysql", "AIToolkit.Tools.Sql.MySql.Sample.Setup");
    await ExecuteNonQueryAsync(setupConnectionString, $"CREATE DATABASE IF NOT EXISTS {EscapeMySqlIdentifier(databaseName)};");

    var databaseConnectionString = BuildConnectionString(adminConnectionString, databaseName, "AIToolkit.Tools.Sql.MySql.Sample.Setup");
