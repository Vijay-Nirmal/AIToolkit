using AIToolkit.Tools.Sql.PostgreSql;
...
if (string.IsNullOrWhiteSpace(apiKey))
{
    ...
    Console.Error.WriteLine("dotnet user-secrets set \"OpenAI:ApiKey\" \"<your-api-key>\" --project samples/AIToolkit.Tools.Sql.PostgreSql.Sample");
    return;
}

await EnsureSampleDatabaseAsync(adminConnectionString, databaseName);

var connectionString = BuildConnectionString(adminConnectionString, databaseName, "AIToolkit.Tools.Sql.PostgreSql.Sample");
...
static async Task EnsureSampleDatabaseAsync(string adminConnectionString, string databaseName)
{
    ...
    var databaseConnectionString = BuildConnectionString(adminConnectionString, databaseName, "AIToolkit.Tools.Sql.PostgreSql.Sample.Setup");
...
{
    await using var connection = new NpgsqlConnection(BuildConnectionString(adminConnectionString, "postgres", "AIToolkit.Tools.Sql.PostgreSql.Sample.Setup"));
