...
/// <summary>
/// Collects internal helpers shared across the generic Deck tool pipeline.
/// </summary>
/// <remarks>
...
/// </remarks>
internal static class DeckSupport
{
    ...
    internal static string FormatNumberedSelection(IReadOnlyList<(int LineNumber, string Line)> lines)
    {
        var builder = new StringBuilder();
        for (var index = 0; index < lines.Count; index++)
        {
            builder.Append(lines[index].LineNumber.ToString(CultureInfo.InvariantCulture));
            builder.Append('\t');
            builder.Append(lines[index].Line);
    ...
    internal static void CompactBlankLines(List<string> lines)
    {
...
/// <summary>
/// Resolves Deck handlers from options and dependency injection.
/// </summary>
/// <remarks>
/// The registry merges handlers supplied directly through <see cref="DeckToolsOptions.Handlers"/> with handlers
/// registered in the active <see cref="IServiceProvider"/>. <see cref="DeckToolService"/> uses it to locate the first
/// handler that can own a resolved Deck reference.
/// </remarks>
internal sealed class DeckHandlerRegistry(DeckToolsOptions options)
{
    private readonly DeckToolsOptions _options = options;
    ...
    /// <summary>
    /// Creates the handler context used for a resolved Deck operation.
    /// </summary>
    public DeckHandlerContext CreateContext(string DeckReference, DeckReferenceResolution resolution, IServiceProvider? services) =>
        new(DeckReference, resolution, _options, services);

    /// <summary>
    /// Resolves the first handler that can process the supplied Deck context.
    /// </summary>
    public IDeckHandler? ResolveHandler(DeckHandlerContext context)
    {
    ...
    private IEnumerable<IDeckHandler> GetHandlers(IServiceProvider? services)
    {
        ...
        if (services?.GetService(typeof(IEnumerable<IDeckHandler>)) is IEnumerable<IDeckHandler> serviceHandlers)
        {
...
/// <summary>
/// Tracks the last canonical DeckDoc snapshot read for each resolved Deck.
/// </summary>
/// <remarks>
/// The generic edit and write flows use this store to reject stale updates and to distinguish full reads from partial
/// reads.
/// </remarks>
internal sealed class DeckReadStateStore
{
    private readonly ConcurrentDictionary<string, DeckReadStateEntry> _entries = new(StringComparer.Ordinal);

    /// <summary>
    /// Retrieves the read-state entry for a resolved Deck key.
    /// </summary>
    public DeckReadStateEntry? Get(string path) =>
        _entries.TryGetValue(path, out var entry) ? entry : null;

    /// <summary>
    /// Stores or replaces the read-state entry for a resolved Deck key.
    /// </summary>
    public void Set(string path, DeckReadStateEntry entry) =>
        _entries[path] = entry;
}

/// <summary>
/// Captures the canonical DeckDoc that was last read together with whether the model saw the full Deck.
/// </summary>
internal sealed record DeckReadStateEntry(
    string NormalizedDeckDoc,
    bool IsPartialView,
    int? Offset,
    int? Limit);

/// <summary>
/// Represents the current canonical DeckDoc snapshot re-read from a provider before a write or edit completes.
/// </summary>
internal sealed record DeckDocSnapshot(
    string NormalizedDeckDoc);

/// <summary>
/// Evaluates a subset of glob patterns needed for Deck file searches.
/// </summary>
/// <remarks>
/// The matcher intentionally supports only the Deck search scenarios used by <see cref="DeckToolService"/>:
