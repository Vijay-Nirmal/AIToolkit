...
internal static class FunctionTestUtilities
{
    ...
    public static IReadOnlyList<AIFunction> CreateFunctions(string? workingDirectory = null, PowerPointDeckHandlerOptions? handlerOptions = null) =>
        PowerPointDeckTools.CreateFunctions(
            handlerOptions,
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), "AIToolkit.Tools.Deck.PowerPoint.Tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        return directory;
    }

    public static string ReadDeckDocText(IReadOnlyList<AIContent> contents)
    {
