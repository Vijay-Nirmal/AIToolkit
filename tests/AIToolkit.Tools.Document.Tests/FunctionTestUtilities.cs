...
internal static class FunctionTestUtilities
{
    ...
    public static IReadOnlyList<AIFunction> CreateFunctions(
        ...
        IEnumerable<IDocumentToolPromptProvider>? promptProviders = null) =>
        DocumentTools.CreateFunctions(
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), "AIToolkit.Tools.Document.Tests", Guid.NewGuid().ToString("N"));
