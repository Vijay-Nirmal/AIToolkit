...
internal static class FunctionTestUtilities
{
    ...
    public static IReadOnlyList<AIFunction> CreateFunctions(string? workingDirectory = null, WordDocumentHandlerOptions? handlerOptions = null) =>
        WordDocumentTools.CreateFunctions(
            handlerOptions,
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), "AIToolkit.Tools.Document.Word.Tests", Guid.NewGuid().ToString("N"));
