...
namespace AIToolkit.Tools.Sql.Sqlite.Tests;

[TestClass]
public class SqliteToolServiceTests
{
    [TestMethod]
    public async Task ExplainQueryAsyncReturnsPlannerSummary()
    {
        var opener = new FakeConnectionOpener(
            static commandText =>
            {
                StringAssert.StartsWith(commandText, "EXPLAIN QUERY PLAN ");
                return CreateReader(
                    [("id", typeof(int)), ("parent", typeof(int)), ("notused", typeof(int)), ("detail", typeof(string))],
                    [0, 0, 0, "SEARCH orders USING COVERING INDEX idx_orders_status (status=?)"],
                    [1, 0, 0, "USE TEMP B-TREE FOR ORDER BY"]);
            });

        var service = CreateToolService(opener, new SqliteQueryClassifier());

        var result = await service.ExplainQueryAsync("local", "select * from orders where status = 'OPEN' order by created_at", "main");

        Assert.IsTrue(result.Success, result.Message);
        Assert.IsNotNull(result.Result);
        Assert.AreEqual(1, opener.OpenCallCount);
        Assert.AreEqual("tabular-json", result.Result.Format);
        Assert.AreEqual("SEARCH", result.Result.RootNode?.NodeType);
        Assert.AreEqual("orders", result.Result.RootNode?.RelationName);
        Assert.AreEqual("idx_orders_status", result.Result.RootNode?.IndexName);
        StringAssert.Contains(result.Result.PlanPayload, "USE TEMP B-TREE FOR ORDER BY");
    }
    ...
    public async Task ExplainQueryAsyncRejectsMutationQueries()
    {
        ...
        var result = await service.ExplainQueryAsync("local", "delete from orders where status = 'OPEN'");
    ...
    public async Task ExplainQueryAsyncRejectsExplainStatements()
    {
        ...
        var result = await service.ExplainQueryAsync("local", "EXPLAIN QUERY PLAN SELECT * FROM orders");

        Assert.IsFalse(result.Success);
        Assert.AreEqual("Provide the underlying SQLite query instead of an EXPLAIN statement.", result.Message);
        Assert.AreEqual(0, opener.OpenCallCount);
    }
    ...
    {
        var opener = new FakeConnectionOpener(static _ => CreateReader([("id", typeof(int)), ("parent", typeof(int)), ("notused", typeof(int)), ("detail", typeof(string))]));
        var service = CreateToolService(opener, new SqliteQueryClassifier());

        var result = await service.ExplainQueryAsync("local", "select * from orders", "main");

        Assert.IsFalse(result.Success);
        Assert.AreEqual("SQLite did not return an EXPLAIN QUERY PLAN result.", result.Message);
        Assert.AreEqual(1, opener.OpenCallCount);
    }

    [TestMethod]
    public async Task ExplainQueryAsyncDefaultsSchemaToMainWhenDatabaseIsNotProvided()
    {
        var opener = new FakeConnectionOpener(static _ => CreateReader(
            [("id", typeof(int)), ("parent", typeof(int)), ("notused", typeof(int)), ("detail", typeof(string))],
            [0, 0, 0, "SCAN orders"]));
        var service = CreateToolService(opener, new SqliteQueryClassifier());

        var result = await service.ExplainQueryAsync("local", "select * from orders");

        Assert.IsTrue(result.Success, result.Message);
        Assert.AreEqual("main", result.Result?.RootNode?.Schema);
        Assert.AreEqual("SCAN", result.Result?.RootNode?.NodeType);
    }

    private static SqliteToolService CreateToolService(FakeConnectionOpener connectionOpener, ISqlQueryClassifier classifier) =>
        new(
...
#pragma warning disable CS8765
    ...
    private sealed class FakeDbConnection(Func<string, DbDataReader> readerFactory) : DbConnection
    {
        ...
        public override string DataSource => "app.db";
        public override string ServerVersion => "3.45";
